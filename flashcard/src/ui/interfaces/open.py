...
class TkinterGUIOpen(TkinterGUITemplate):
    ...
    def __init__(self, window, service, handler):
        # Fontit
        self._font_packlist_li = Font(family="Arial", size=9)
        self._font_packlist_li_hover = Font(family="Arial", size=9, underline=1)
    ...
    def __initialize(self):
        ...
        label = ttk.Label(frame, text=f"Korttipakat kansiossa /{self._service.packfolder}/", font=self._font_h1)
        ...
        for i, file in enumerate(files):
            label = ttk.Label(listframe, text=file, font=self._font_packlist_li)
            label.file = file

            self._add_elem(label, 0, i, sticky="NW")
            label.bind("<Enter>", lambda e: e.widget.config(font=self._font_packlist_li_hover))
            label.bind("<Leave>", lambda e: e.widget.config(font=self._font_packlist_li))
    ...
    def _handle_listframe_elem_click(self, args):
        ...
        self._handler.add_menu(TkinterGUIPack(self._window, self._service, self._handler, path))
