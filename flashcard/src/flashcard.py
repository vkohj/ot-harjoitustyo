...
class Flashcard:
    ...
    def get_next_card(self):
        """Seuraten Flashcard.__packorder listaa, palauttaa seuraavan kortin käytössä olevasta pakasta.
    ...
    def __load_setting_file(self, path):
        ...
        changed = max(changed, self.__setting_set_if_none("font_sentence", "12"))
        changed = max(changed, self.__setting_set_if_none("font_reading", "11"))
        changed = max(changed, self.__setting_set_if_none("font_translation", "11"))
    ...
    def __setting_set_if_none(self, key, value):
        """Lisää asetuksen sanakirjaan Flashcard.__setting, jos siellä ei ole jo samannimistä asetusta.
    ...
    def __pack_set(self):
        """Luokan sisäinen metodi, joka ajetaan aina ennen korttien muokkaamista.
         Tarkastaa, onko käytössä oleva pakka olemassa, ja laittaa muutosta ilmaisevan muuttujan todeksi.

        Returns:
            boolean: True, jos pakka oli olemassa ja Pack.unsaved_changes on muutettu todeksi, muuten False.
        """
    ...
    def set_card_reading(self, index, value):
        ...
        Args:
            index (int): Kortin lauseen sijainti Flashcard.get_sentences()-metodin palauttamassa listassa.
